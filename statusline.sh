...
# Note: Intentionally no 'set -euo pipefail' - statusline must never fail
...
CONTEXT_USED=$(echo "$INPUT" | jq -r '.context.used // 0' 2>/dev/null || echo "0")
CONTEXT_TOTAL=$(echo "$INPUT" | jq -r '.context.total // 1' 2>/dev/null || echo "1")
OUTPUT_STYLE=$(echo "$INPUT" | jq -r 'if .output_style | type == "object" then .output_style.name else .output_style end // "default"' 2>/dev/null || echo "default")
