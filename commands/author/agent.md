---
description: Guide for authoring specialized AI agents with focused expertise and tool restrictions
---

# author/agent

Guides you through creating a new AI agent with specialized expertise.

**Usage:** `/author:agent [agent-name]`

**Delegation:** Invokes the **agent-author** skill for comprehensive interactive guidance on purpose definition, model selection, tool restrictions, focus areas, approach, description crafting, and validation.
