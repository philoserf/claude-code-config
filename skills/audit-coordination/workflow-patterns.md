...
2. config-audit (secondary) - Structure and metadata
3. test-runner (optional) - Functional validation

**Sequence**: Sequential (audit-skill → config-audit → test-runner)
...
````text
User: "Audit my version-control skill comprehensively"
...
2. config-audit (optional) - Structure validation

**Sequence**: Sequential (audit-hook → config-audit if needed)

**Why Sequential**: Initial audit findings determine if config-audit needed
...
Step 2: (Conditional) Invoke config-audit if structural issues found
...
1. config-audit only (no specialized auditor exists)
...
2. config-audit for overall assessment

**Sequence**: Parallel for skills, then config-audit
...
1. config-audit (setup-wide analysis)
...
Step 1: Invoke config-audit for overall assessment
...
- One auditor's output informs another (audit-skill → config-audit)
...
audit-skill finds critical discovery issues
  ↓
config-audit confirms structural problems
  ↓
...
For agents like config-audit and test-runner:

```python
# Using Task tool
Task(
    subagent_type="config-audit",
    prompt="Audit the version-control skill for structure and correctness",
    description="Audit version-control skill"
)
```text

### Pattern: Skill Tool Invocation (Skills)

For skills like editing-assistant:
...
For setup-wide audits, consider early exit if config-audit finds critical issues:

```text
config-audit finds: "settings.json has syntax errors"
...
| ----------------------------- | -------------------------------------- | ---------- | -------- | -------------------- |
| Single skill (comprehensive)  | audit-skill, config-audit, test-runner  | Sequential | 30-90s   | Full skill analysis  |
| Single skill (discovery only) | audit-skill                          | Single     | 10-20s   | Quick check          |
| Single hook                   | audit-hook, config-audit (optional)     | Sequential | 15-30s   | Hook safety check    |
| Single agent/command          | config-audit                              | Single     | 15-30s   | Structure validation |
| All skills                    | audit-skill (each), config-audit        | Parallel   | 60-120s  | Batch skill audit    |
| All hooks                     | audit-hook (each)                    | Parallel   | 30-60s   | Batch hook audit     |
| Complete setup                | config-audit, audit-skill, audit-hook | Parallel   | 90-180s  | Comprehensive audit  |
