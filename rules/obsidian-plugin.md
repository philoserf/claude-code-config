...
- Releasing means `release-gate` first, then `obsidian-ship`. Never tag or publish by hand: no bare `git tag`, no `gh release create`, and never `npm version` / `bun version` (they auto-tag and skip the CHANGELOG and walkthrough steps).
