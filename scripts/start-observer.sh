...
# Check for claude CLI (required for analysis)
if ! command -v claude &>/dev/null; then
	echo "Error: claude CLI not found."
	echo "Install from https://claude.ai/code or https://github.com/anthropics/claude-code"
	exit 1
fi
...
start)
	...
	# The observer loop
	(
		# Disable errexit in subshell - we handle errors manually
		set +e
		...
		# Get actual subshell PID ($$  gives parent in bash 3.x)
		MY_PID=$(sh -c 'echo $PPID')
		echo "$MY_PID" >"$PID_FILE"
		echo "[$(date)] Observer started (PID: $MY_PID)" >>"$LOG_FILE"
